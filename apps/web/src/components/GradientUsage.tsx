...
export function GradientUsage({
  ...
  const [selectedTab, pSetSelectedTab] = useState<tabs>(
    'CSS'
  );
  ...
  useEffect(() => {
    const urlTab =
      new URLSearchParams(window.location.search).get('usage') as tabs | null;
        ...
        <div className='flex flex-col gap-8'>
          <div>
            <button
              ...
              onClick={() =>
                void navigator.clipboard.writeText(`https://gradient.tips/api/v1/g/${gradient.id}.json`)
              }
            >
            ...
            <div class='form-control'>
              <label className='label cursor-pointer'>
                <span class='label-text'>Prettify output with <code>jq</code> utility</span>
                <input
                  type='checkbox'
          ...
          <div className='flex flex-col gap-2'>
            ...
            <pre>
              <code>
               curl -s -H "Content-Type: application/json" https://gradient.tips/api/v1/g/{gradient.id}.json{showAPIJqExample ? ' | jq' : ''}
              </code>
            </pre>
            </div>
          ...
          <div className='flex flex-col gap-2'>
            ...
            <pre>
              <code>
               wget -qO- https://gradient.tips/api/v1/g/{gradient.id}.json{showAPIJqExample ? ' | jq' : ''}
              </code>
            </pre>
            </div>
            <div class='flex flex-col max-w-xs sm:max-w-sm md:flex-row gap-2'>
          ...
          <div className='flex flex-col gap-2'>
            ...
            {!showAPIJqExample && <div
              className='mockup-code max-w-xs sm:max-w-sm md:max-w-lg lg:max-w-xl xl:max-w-3xl 2xl:max-w-4xl'
              id='api-response-code'
            >
              <pre><code>{`{"id":"${gradient.id}","name":"${gradient.name}","colors":[${gradient.colors.join(', ')}]}%`}</code></pre>
            </div>}
            ...
            >
              <pre><code>{`{`}</code></pre>
              <pre><code>{`  "id": "${gradient.id}",`}</code></pre>
              <pre><code>{`  "name": "${gradient.name}",`}</code></pre>
              <pre><code>{`  "colors": [`}</code></pre>
              {gradient.colors.map((color, index) =>
                <pre><code>{`    "${color}"${index < gradient.colors.length - 1 ? ',' : ''}`}</code></pre>)}
              <pre><code>{`  ]`}</code></pre>
              <pre><code>{`}`}</code></pre>
            </div>}
          </div>
